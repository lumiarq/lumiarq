...
<body :class="{ 'sidebar-open': sidebarOpen }"
  x-data="{
...
<header class="site-header">
  ...
  <nav class="header-nav" aria-label="${__t('aria.documentation_nav')}">
    <a href="`
  __o += __e(site.nav.docs)
  __o += `">${__t('nav.documentation')}</a>
    <a href="`
  __o += __e(site.github)
  __o += `" target="_blank" rel="noopener">${__t('nav.github')}</a>

    <button @click="searchOpen = true" class="icon-btn search-btn" aria-label="${__t('aria.search')}">
      <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
        <circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/>
      </svg>
    </button>

    <button @click="toggleTheme()" class="icon-btn" aria-label="${__t('aria.theme_toggle')}">
  ...
  __o += `" class="btn-primary">${__t('nav.get_started')}</a>
  </nav>

  <button @click="sidebarOpen = !sidebarOpen" class="hamburger-btn" aria-label="${__t('aria.toggle_nav')}">
...
<div class="layout mx-auto w-full max-w-[1440px] px-4 sm:px-6 lg:px-8">
  <nav class="sidebar"
       @click="if($event.target.closest('.sidebar-link')) sidebarOpen = false"
       aria-label="${__t('aria.documentation_nav')}">
    <!-- ═══════════════════════════════════════════════════════════════════════════
...
<div class="version-selector">
  <label class="version-label" for="version-switcher">${__t('docs.version_label')}</label>
